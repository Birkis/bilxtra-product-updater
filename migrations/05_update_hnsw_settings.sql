-- Drop existing index
DROP INDEX IF EXISTS car_details_vector_embedding_hnsw_idx;

-- Recreate with optimized settings for better recall
CREATE INDEX car_details_vector_embedding_hnsw_idx 
ON car_details_vector 
USING hnsw (embedding vector_cosine_ops)
WITH (
    m = 32,              -- Increased from 16 to allow more connections per layer
    ef_construction = 128 -- Increased from 64 to consider more candidates during construction
);

-- Update the HNSW search function to be more lenient
...
BEGIN
    ...
    SELECT
        ...
        (vm.similarity * 0.4 +  -- Reduced weight on similarity score
         (CASE WHEN vm.front_rack_id IS NOT NULL THEN 2.0 ELSE 0.0 END +
          CASE WHEN vm.rear_rack_id IS NOT NULL THEN 2.0 ELSE 0.0 END +
          CASE WHEN vm.bar_id IS NOT NULL THEN 1.0 ELSE 0.0 END +
          CASE WHEN vm.foot_id IS NOT NULL THEN 1.0 ELSE 0.0 END +
          ...
          CASE WHEN vm.compatible_rack_for_tent THEN 0.2 ELSE 0.0 END +
          CASE WHEN vm.compatible_roof_for_tent THEN 0.2 ELSE 0.0 END
         )::float / 8.0 * 0.6  -- Increased weight on product matches
        ) AS combined_score
