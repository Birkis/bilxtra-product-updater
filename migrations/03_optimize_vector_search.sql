...
CREATE INDEX IF NOT EXISTS idx_car_data_make ON car_data (make);
CREATE INDEX IF NOT EXISTS idx_car_data_model ON car_data (model);
CREATE INDEX IF NOT EXISTS idx_car_data_years ON car_data (car_start_year, car_stop_year);

-- Drop existing optimized function if it exists
DROP FUNCTION IF EXISTS match_car_details_optimized;

-- Create new optimized function
CREATE OR REPLACE FUNCTION match_car_details_optimized(
...
BEGIN
    ...
    WITH base_matches AS (
        -- First filter by exact matches on make, model, and year
        SELECT c.*
        FROM car_data c
        WHERE 
    ...
    vector_matches AS (
        -- Then calculate vector similarity only on the filtered subset
        SELECT 
        ...
        ORDER BY 1 - (cd.embedding <-> query_embedding) DESC
        LIMIT match_count
    )
    SELECT
